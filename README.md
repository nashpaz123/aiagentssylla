...
- **החל ממפגש 2:** כל בלוק קוד / פסאודוקוד במצגת המרצה חייב להופיע גם במצגת הסטודנטים (באותו מספר).
- **יעד משך לכל מפגש:** **יותר משעתיים**, יעד עבודה **~2.5–3 שעות** מוקלטות (הוראה + הדגמה/Lab + תרגילים).  
  אם המפגש יוצא ~שעה — חסר עומק/דוגמאות/Lab/תרגילים; יש לעבות לפני הקלטה.
- **מפגש 2:** כולל Lab חי (`labs/02-aws-agent-loop/`) — Claude Code + AWS read-only (~20 דק׳) + תרגילים (~50 דק׳).  
  חזרה על מפגש 1 — רק מצביע קצר, בלי פירוט מחדש.
